...
class db_class(object):
    ...
    def select_count(self, table_name: str, condition: list = []) -> int:
        '''
        条件查询\n
        ...
        try:
            ...
            print(e)
            print(data)
            self.mylog.logger.warning(e)
            return None
        finally:
            cur.close()
            con.close()

    def select_condition(self, table_name, condition) -> list:
        '''
        条件查询\n
    ...
    def select_LIMIT(self, table_name, start, end):
        try:
            con = self.pool.connection()
            cur = con.cursor()
            _sql = 'SELECT top ' + end + ' * from ' + table_name + ' where IsChange=0 and id not in (SELECT top ' + start + ' id from ' + table_name + '  where IsChange=0);'
            cur.execute(_sql)
            result = cur.fetchall()
            print(','.join([str(r[0]) for r in result]))
            _sql = 'update ' + table_name + ' set IsChange=9 where id in (' + ','.join(
...
if __name__ == '__main__':
    db = db_class()
    result = db.select_condition('eol_score', [['[school_id]', '=', 140], ['name', 'like', '%清华%']])
    print(result)
